...
$result = shell_exec("python doc-1.py \"$practManager\" \"$practManager\" \"$practManager\" \"$studFnp\" \"$instName\" \"$groupCourse\" \"$groupName\" \"ФГБОУ ВО 'Югорский Государственный Университет'\" \"$postPM\" \"$typePrac\" \"$date\" \"$postPM\" \"$dirName\" \"$postPM\" \"$viewPrac\" \"$address\" \"$quality\" \"$difficult\" \"$volume\" \"$comment\" \"$mark\" \"$idStud\" ");
